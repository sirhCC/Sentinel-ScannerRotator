...
export async function createIssues(
  ...
  const minSevStr = (opts?.minSeverity ||
    (process.env.SENTINEL_ISSUES_MIN_SEVERITY as any) ||
  ...
  if (provider === 'file') {
    const filePath =
      opts?.filePath ||
      process.env.SENTINEL_ISSUES_FILE ||
      path.join(process.cwd(), '.sentinel_issues.ndjson');
    const issues: Issue[] = filtered.map((f) => ({
      ts: Date.now(),
      file: f.filePath,
      line: f.line,
      column: f.column,
  ...
  if (provider === 'github') {
    ...
    const lines = filtered.map(
      (f) =>
        `- ${f.severity?.toUpperCase() || 'MEDIUM'}: ${f.ruleName || 'Rule'} in ${f.filePath}:${f.line}`,
    );
    const body = [
    ...
    const headers: Record<string, string> = {
      ...
      'User-Agent': 'SecretSentinel',
    };
    if (token) headers['Authorization'] = `Bearer ${token}`;
    const res = await fetch(url, {
      method: 'POST',
      headers,
      body: JSON.stringify({ title, body, labels: ['security', 'secret-detection'] }),
    });
    if (!res.ok) {
      ...
      return {
        created: 0,
        provider: 'github',
        error: `GitHub API error ${res.status}: ${txt}`,
      } as any;
    }
