...
export async function runCli(
  ...
  try {
    ...
    program
      ...
      .option(
        '-t, --template <tpl>',
        'replacement template for apply (supports {{match}}, {{timestamp}}, {{file}})',
      )
      ...
      .option(
        '--cache <path>',
        'persist scan cache to a file to speed up repeated runs (or use SENTINEL_CACHE env)',
      )
      .option(
        '--scan-concurrency <n>',
        'number of concurrent file scans (default 8 or SENTINEL_SCAN_CONCURRENCY)',
        (v) => parseInt(v, 10),
      )
      .option(
        '--rotate-concurrency <n>',
        'number of concurrent rotations (default 4 or SENTINEL_ROTATE_CONCURRENCY)',
        (v) => parseInt(v, 10),
      )
      .option('--list-rulesets', 'list available curated rulesets and exit', false)
      .option(
        '--rulesets <names>',
        'comma- or space-separated curated rulesets to enable (also via SENTINEL_RULESETS env)',
      )
      .option(
        '--rulesets-dirs <dirs>',
        'comma-separated directories to discover external *.ruleset.json files (SENTINEL_RULESETS_DIRS)',
      )
      .option(
        '--rulesets-catalog <urlOrFile>',
        'ruleset marketplace catalog (http(s):// or file path)',
      )
      .option(
        '--rulesets-install <names>',
        'install rulesets from catalog into cache dir (comma- or space-separated)',
      )
      .option(
        '--rulesets-cache-dir <dir>',
        'cache directory for installed rulesets (default ./.sentinel_rulesets)',
      )
      .option(
        '--rulesets-require-signed',
        'require ed25519 signatures for installed rulesets',
        false,
      )
      .option(
        '--rulesets-pubkey <pemOrPath>',
        'PEM public key for verifying signed rulesets (or set SENTINEL_RULESET_PUBKEY)',
      )
      .option(
        '--rulesets-catalog-require-signed',
        'require detached signature for catalog (catalog.json.sig)',
        false,
      )
      .option('--rulesets-catalog-pubkey <pemOrPath>', 'PEM public key to verify catalog signature')
      .option(
        '--disable-builtin-rules',
        'disable built-in rules (SENTINEL_DISABLE_BUILTIN_RULES=true)',
        false,
      )
      .option('--issues', 'auto-create issues on fail-on-findings (file provider default)', false)
      .option(
        '--issues-file <path>',
        'issues file path for file provider (.sentinel_issues.ndjson default)',
      )
      ...
      .option(
        '--show-runtime-info',
        'print runtime config (engine, workers, cache, concurrency, version) and exit',
        false,
      )
      .option(
        '--fail-on-findings',
        'exit non-zero if any findings are found (skips rotation)',
        false,
      )
      .option(
        '--fail-threshold <n>',
        'exit non-zero if findings exceed N (with --fail-on-findings)',
        (v) => parseInt(v, 10),
      )
      .option(
        '--fail-threshold-high <n>',
        'with --fail-on-findings: fail if HIGH severity findings exceed N',
        (v) => parseInt(v, 10),
      )
      .option(
        '--fail-threshold-medium <n>',
        'with --fail-on-findings: fail if MEDIUM severity findings exceed N',
        (v) => parseInt(v, 10),
      )
      .option(
        '--fail-threshold-low <n>',
    ...
    const rotateConcOpt =
      opts.rotateConcurrency ?? (isNaN(rotateConcEnv) ? undefined : rotateConcEnv);
    ...
    if (opts.rulesetsCatalog && opts.rulesetsInstall) {
      ...
      const names = String(opts.rulesetsInstall)
        .split(/[,;\s]+/)
        .filter(Boolean);
    ...
    if (opts.listRulesets) {
      try {
        ...
        const names = await listRulesets(
          (process.env.SENTINEL_RULESETS_DIRS || '').split(/[,;]+/).filter(Boolean),
        );
    ...
    if (rotator.name === 'apply' && !opts.dryRun && !opts.force && !opts.interactive) {
      logger.error(
        "Refusing to run 'apply' without --dry-run or --force. Use --force to confirm destructive changes.",
      );
      return 3;
    }
    ...
    async function exportFindingsIfRequested() {
      ...
      if (fmt === 'json') {
        const minimized = findings.map((f: any) => ({
          file: f.filePath,
          line: f.line,
          column: f.column,
          match: f.match,
        ...
        const rows = findings.map((f: any) =>
          [esc(f.filePath), f.line, f.column, esc(f.match)].join(','),
        );
    ...
    if (opts.failOnFindings) {
      ...
      if ((opts as any).minSeverity && !cliMin) {
        logger.warn(
          `Ignoring invalid --min-severity=${JSON.stringify((opts as any).minSeverity)} (expected low|medium|high)`,
        );
      }
      const minSeverity: 'low' | 'medium' | 'high' | undefined =
        cliMin ?? validSev(policy?.minSeverity);
      if (!cliMin && policy?.minSeverity && !minSeverity) {
        logger.warn(
          `Ignoring invalid policy.minSeverity=${JSON.stringify(policy.minSeverity)} (expected low|medium|high)`,
        );
      }
      ...
      const considered = minSeverity
        ? findings.filter(
            (f: any) =>
              rank[String(f.severity || 'medium').toLowerCase() as 'low' | 'medium' | 'high'] >=
              rank[minSeverity],
          )
        : findings;
      // Per-severity thresholds (only enforced if provided) computed on considered set
      const sevCounts = considered.reduce(
      ...
      if (policy?.forbidRules && Array.isArray(policy.forbidRules) && policy.forbidRules.length) {
        const hit = findings.find(
          (f: any) => f.ruleName && policy.forbidRules.includes(f.ruleName),
        );
        if (hit) {
          logger.error(
            `Failing due to forbidden rule matched: ${hit.ruleName} in ${hit.filePath}:${hit.line}`,
          );
          return 4;
        }
      }
      const checkSev = (name: 'high' | 'medium' | 'low', thr: number | undefined) => {
        ...
        if (n > (thr as number)) {
          logger.error(
            `Failing due to ${name.toUpperCase()} severity findings (${n}) exceeding threshold (${thr}).`,
      ...
      const thrHigh = Number.isFinite(opts.failThresholdHigh)
        ? opts.failThresholdHigh
        : policy?.thresholds?.high;
      const thrMed = Number.isFinite(opts.failThresholdMedium)
        ? opts.failThresholdMedium
        : policy?.thresholds?.medium;
      const thrLow = Number.isFinite(opts.failThresholdLow)
        ? opts.failThresholdLow
        : policy?.thresholds?.low;
      const tripped =
        checkSev('high', thrHigh) || checkSev('medium', thrMed) || checkSev('low', thrLow);
      if (tripped) {
        if (opts.issues) {
          try {
            const { createIssues } = await import('./issues.js');
            const provider = (opts.issuesProvider || process.env.SENTINEL_ISSUES_PROVIDER) as
              | string
              | undefined;
        ...
        if (opts.metrics) {
          try {
            await writeProm(m, opts.metrics);
          } catch {}
        }
        return 4;
      }
      const threshold: number = Number.isFinite(opts.failThreshold)
        ? opts.failThreshold
        : (policy?.thresholds?.total ?? 0);
      if (considered.length > threshold) {
        logger.error(
          `Failing due to findings (${considered.length}) exceeding threshold (${threshold}).`,
          minSeverity ? { minSeverity } : undefined,
        );
        if (opts.issues) {
          try {
    ...
    async function shouldApplyForFinding(f: any) {
      ...
      const answer = (
        await q(`Apply ${rotator.name} to ${f.filePath}:${f.line} match="${f.match}" ? [y/N] `)
      )
        .trim()
        .toLowerCase();
      rl.close();
    ...
    const rotateConc = Math.max(
      1,
      opts.rotateConcurrency ?? (isNaN(envRotateConc) ? undefined : envRotateConc) ?? 4,
    );
    let fi = 0;
    async function rotateWorker() {
      while (true) {
        ...
        const hasBatch = (
          ...
          name: string;
          rotateFile: (filePath: string, findings: any[], options?: any) => Promise<any[]>;
        } => typeof r?.rotateFile === 'function';
        if (hasBatch(rotator)) {
          ...
          const results = await rotator.rotateFile(file, group, {
            dryRun,
          ...
          if (ok) m.rotations_success++;
          else m.rotations_failed++;
          const msg =
            results[0]?.message ||
            (ok
              ? `Rotated ${group.length} findings in ${file}`
              : `Failed to rotate findings in ${file}`);
          if (ok) logger.info(msg as string);
          else logger.warn(msg as string);
          if (auditor) {
          ...
          for (const f of group) {
            ...
            m.rotations_total++;
            if (res.success) m.rotations_success++;
            else m.rotations_failed++;
    ...
    const workers = Array.from({ length: Math.min(rotateConc, files.length || 1) }, () =>
      rotateWorker(),
    );
