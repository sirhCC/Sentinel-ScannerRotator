...
describe('ML hook integration', () => {
  it('invokes analyzeLine and yields extra findings', async () => {
    ...
    fs.writeFileSync(hookFile, `export function analyzeLine(line, ctx) { if (line.includes('TOKEN_XYZ')) return [{ token: 'TOKEN_XYZ_12345', index: line.indexOf('TOKEN_XYZ_12345'), ruleName: 'ML-Test', severity: 'low' }]; }`);
