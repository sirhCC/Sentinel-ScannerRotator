...
describe('backend roll-forward (file provider)', () => {
  it('updates existing secret value with key override and logs history', async () => {
    ...
    fs.writeFileSync(
      cfgPath,
      JSON.stringify({
        patterns: [{ name: 'AWS Access Key ID', regex: 'AKIA[0-9A-Z]{16}', severity: 'high' }],
      }),
    );
    ...
    const code = await runCli(
      ...
      { ...backendEnv, SENTINEL_BACKEND_KEY_OVERRIDE: key },
    );
    expect(code).toBe(0);
    ...
    const prevEnv = {
      A: process.env.SENTINEL_BACKEND,
      B: process.env.SENTINEL_BACKEND_FILE,
      C: process.env.SENTINEL_BACKEND_KEY_OVERRIDE,
    };
    ...
    const res = await backendRotator.rotate(
      { filePath: file, line: 1, column: 1, match: 'AKIAZZZZZZZZZZZZZZ' } as any,
      { verify: true },
    );
