...
describe('ML hook timeout and metrics', () => {
  it('applies timeout and increments metrics', async () => {
    ...
    fs.writeFileSync(
      hookFile,
      `export async function analyzeLine(line, ctx) { await new Promise(r=>setTimeout(r, 10)); return [{ token: 'TOKEN_123', index: line.indexOf('TOKEN_123'), ruleName: 'ML-Slow', severity: 'low' }]; }`,
    );
